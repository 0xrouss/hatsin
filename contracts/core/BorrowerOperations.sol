...
/**
    @title Hatsin Borrower Operations
    @notice Based on Liquity's `BorrowerOperations`
            ...
            Hatsin's implementation is modified to support multiple collaterals. There is a 1:n
            relationship between `BorrowerOperations` and each `DenManager` / `SortedDens` pair.
 */
contract BorrowerOperations is HatsinBase, HatsinOwnable, DelegatedOps {
    ...
    ) HatsinOwnable(_hatsinCore) HatsinBase(_gasCompensation) DelegatedOps(_hatsinCore) {
        if (_hatsinCore == address(0) || _debtTokenAddress == address(0) || _factory == address(0)) {
    ...
    function openDen(
        ...
        require(!HATSIN_CORE.paused(), "Deposits are paused");
        IERC20 collateralToken;
        ...
        vars.ICR = HatsinMath._computeCR(_collateralAmount, vars.compositeDebt, vars.price);
        vars.NICR = HatsinMath._computeNominalCR(_collateralAmount, vars.compositeDebt);
    ...
    function addColl(
        ...
        require(!HATSIN_CORE.paused(), "Den adjustments are paused");
    ...
    function withdrawDebt(
        ...
        require(!HATSIN_CORE.paused(), "Withdrawals are paused");
    ...
    function adjustDen(
        ...
        require((_collDeposit == 0 && !_isDebtIncrease) || !HATSIN_CORE.paused(), "Den adjustments are paused");
    ...
    function _triggerBorrowingFee(
        ...
        debtToken.mint(HATSIN_CORE.feeReceiver(), debtFee);
    ...
    function _requireValidAdjustmentInCurrentMode(
        ...
        uint256 oldICR = HatsinMath._computeCR(_vars.coll, _vars.debt, _vars.price);
    ...
    function _requireICRisAboveCCR(uint256 _newICR) internal view {
        require(_newICR >= HATSIN_CORE.CCR(), "BorrowerOps: Operation must leave den with ICR >= CCR");
    }
    ...
    function _requireNewTCRisAboveCCR(uint256 _newTCR) internal view {
        require(_newTCR >= HATSIN_CORE.CCR(), "BorrowerOps: An operation that would result in TCR < CCR is not permitted");
    }
    ...
    function _getNewICRFromDenChange(
        ...
        uint256 newICR = HatsinMath._computeCR(newColl, newDebt, _price);
        return newICR;
    }
    ...
    function _getNewTCRFromDenChange(
        ...
        uint256 newTCR = HatsinMath._computeCR(totalColl, totalDebt);
        return newTCR;
    }
    ...
    function _getTCRData(
        ...
        amount = HatsinMath._computeCR(totalPricedCollateral, totalDebt);
