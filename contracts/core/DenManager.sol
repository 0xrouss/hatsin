...
import "../dependencies/HatsinBase.sol";
import "../dependencies/HatsinMath.sol";
import "../dependencies/HatsinOwnable.sol";

/**
    @title Hatsin Den Manager
    @notice Based on Liquity's `TroveManager`
            https://github.com/liquity/dev/blob/main/packages/contracts/contracts/TroveManager.sol

            Hatsin's implementation is modified so that multiple `DenManager` and `SortedDens`
 ...
 */
contract DenManager is HatsinBase, HatsinOwnable, SystemStart {
    ...
    ) HatsinOwnable(_hatsinCore) HatsinBase(_gasCompensation) SystemStart(_hatsinCore) {
        if (_hatsinCore == address(0) || _gasPoolAddress == address(0) || _debtTokenAddress == address(0) || _borrowerOperations == address(0) || _liquidationManager == address(0)) {
    ...
    function setParameters(IFactory.DeploymentParams calldata params) public  {
        ...
        require(params.MCR <= HATSIN_CORE.CCR() && params.MCR >= 1.1e18, "MCR cannot be > CCR or < 110%");
    ...
    function collectInterests() external {
        ...
        debtToken.mint(HATSIN_CORE.feeReceiver(), interestPayableCached);
        interestPayable = 0;
    }
    ...
    function fetchPrice() public view returns (uint256) {
        ...
        if (address(_priceFeed) == address(0)) {
            _priceFeed = IPriceFeed(HATSIN_CORE.priceFeed());
        }
    ...
    function getNominalICR(address _borrower) public view returns (uint256) {
        ...
        uint256 NICR = HatsinMath._computeNominalCR(currentCollateral, currentDebt);
        return NICR;
    }
    ...
    function getCurrentICR(address _borrower, uint256 _price) public view returns (uint256) {
        ...
        uint256 ICR = HatsinMath._computeCR(currentCollateral, currentDebt, _price);
        return ICR;
    }
    ...
    function _updateBaseRateFromRedemption(
        ...
        newBaseRate = HatsinMath._min(newBaseRate, DECIMAL_PRECISION); // cap baseRate at a maximum of 100%
    ...
    function _calcBorrowingRate(uint256 _baseRate) internal view returns (uint256) {
        return HatsinMath._min(borrowingFeeFloor + _baseRate, maxBorrowingFee);
    }
    ...
    function _calcDecayedBaseRate() internal view returns (uint256) {
        ...
        uint256 decayFactor = HatsinMath._decPow(minuteDecayFactor, minutesPassed);
    ...
    function redeemCollateral(
        ...
        require(block.timestamp >= systemDeploymentTime + HATSIN_CORE.dmBootstrapPeriod(), "BOOTSTRAP_PERIOD");
        ...
        _sendCollateral(HATSIN_CORE.feeReceiver(), totals.collateralFee);
    ...
    function _redeemCollateralFromDen(
        ...
        singleRedemption.debtLot = HatsinMath._min(_maxDebtAmount, t.debt - DEBT_GAS_COMPENSATION);
        ...
        if (newDebt == DEBT_GAS_COMPENSATION) {
            ...
            uint256 newNICR = HatsinMath._computeNominalCR(newColl, newDebt);
            /*
    ...
    function claimCollateral(address borrower, address _receiver) external {
        // Only allow users to claim their own collateral
        require(msg.sender == borrower, "DenManager: Only borrower can claim");
    ...
    function _applyPendingRewards(address _borrower) internal returns (uint256 coll, uint256 debt) {
        ...
        if (t.status == Status.active) {
            ...
            coll = t.coll;
            // We accrue interests for this den if not already updated
            if (denInterestIndex < currentInterestIndex) {
