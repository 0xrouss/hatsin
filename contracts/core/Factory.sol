...
contract Factory is HatsinOwnable {
    ...
    ) HatsinOwnable(_hatsinCore) {
        if (_hatsinCore == address(0) || address(_debtToken) == address(0) || address(_liquidStabilityPool) == address(0) || address(_borrowerOperations) == address(0) || _sortedDens == address(0) || _denManager == address(0) || address(_liquidationManager) == address(0)) {
