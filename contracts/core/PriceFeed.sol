...
/**
    @title Hatsin Multi Token Price Feed
 ...
 */
contract PriceFeed is HatsinOwnable {
    ...
    constructor(address _metaHatsinCore) HatsinOwnable(_metaHatsinCore) {
    ...
    modifier onlyOwnerOrManager() {
        require(msg.sender == owner() || msg.sender == HATSIN_CORE.manager(), "Only owner or manager");
        _;
    }
    ...
    function fetchPrice(address _token) public view returns (uint256) {
        if (feedType[_token].isCollVault) {
            return IBaseCollateralVault(_token).fetchPrice();
        }
    ...
    function _isPriceChangeAboveMaxDeviation(
        ...
        uint256 minPrice = HatsinMath._min(currentScaledPrice, prevScaledPrice);
        uint256 maxPrice = HatsinMath._max(currentScaledPrice, prevScaledPrice);

        uint256 percentDeviation = ((maxPrice - minPrice) * HatsinMath.DECIMAL_PRECISION) / prevScaledPrice;
