...
/*  Helper contract for grabbing LSP data from offchain. Not part of the core Hatsin system. */
contract LiquidStabilityPoolGetters {
    ...
    function hatsinCore() external view returns (address) {
        bytes32[] memory slot = _array(LSPStorageLib.metaHatsinCoreSlot());
    ...
    function acceptedStables() public view returns (address[] memory res) {
        bytes32[] memory slot = _array(LSPStorageLib.acceptedStablesValuesSlot());
        uint256 len = uint(lsp.extSloads(slot)[0]);

        res = new address[](len);
        for (uint256 i; i < len; i++) {
            slot = _array(bytes32(LSPStorageLib.acceptedStablesArrayValuesSlot(i)));
            res[i] = address(uint160(uint(lsp.extSloads(slot)[0])));
        }
    }
