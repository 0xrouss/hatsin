...
contract MultiCollateralHintHelpers is HatsinBase {
    IBorrowerOperations public immutable borrowerOperations;

    constructor(address _borrowerOperationsAddress, uint256 _gasCompensation) HatsinBase(_gasCompensation) {
        ...
        while (currentDenuser != address(0) && remainingDebt > 0 && _maxIterations-- > 0) {
            ...
            if (netDebt > remainingDebt) {
                if (netDebt > minNetDebt) {
                    uint256 maxRedeemableDebt = HatsinMath._min(remainingDebt, netDebt - minNetDebt);
                    ...
                    partialRedemptionHintNICR = HatsinMath._computeNominalCR(newColl, compositeDebt);
    ...
    */
    function getApproxHint(
        ...
        diff = HatsinMath._getAbsoluteDifference(_CR, denManager.getNominalICR(hintAddress));
        ...
        while (i < _numTrials) {
            ...
            uint256 currentDiff = HatsinMath._getAbsoluteDifference(currentNICR, _CR);
    ...
    function computeNominalCR(uint256 _coll, uint256 _debt) external pure returns (uint256) {
        return HatsinMath._computeNominalCR(_coll, _debt);
    }

    function computeCR(uint256 _coll, uint256 _debt, uint256 _price) external pure returns (uint256) {
        return HatsinMath._computeCR(_coll, _debt, _price);
    }
}
