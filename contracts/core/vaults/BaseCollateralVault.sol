...
import {IMetaHatsinCore} from "../../interfaces/core/IMetaHatsinCore.sol";
...
abstract contract BaseCollateralVault is ERC4626Upgradeable, UUPSUpgradeable, IBaseCollateralVault {
    ...
    function __BaseCollateralVault_init_unchained(BaseInitParams calldata params) internal onlyInitializing {
        ...
        if (address(params._metaHatsinCore) == address(0) || address(params._asset) == address(0)) {
            revert("CollVault: 0 address");
        }

        _requireAssetFeed(params._metaHatsinCore, address(params._asset));
        ...
        $._metaHatsinCore = params._metaHatsinCore;

        __ERC20_init(params._sharesName, params._sharesSymbol);
        __ERC4626_init(params._asset);
    }

    function _requireAssetFeed(IMetaHatsinCore _metaHatsinCore, address asset) internal virtual {
        IPriceFeed priceFeed = IPriceFeed(_metaHatsinCore.priceFeed());
    ...
    function _onlyOwner() private view {
        ...
        require(msg.sender == getMetaHatsinCore().owner(), "CollVault: caller is not the owner");
    }
    ...
    function _applyShareFee(uint shares, uint _totalSupply, uint fee) internal virtual returns (uint, uint) {
        ...
        address feeReceiver = $._metaHatsinCore.feeReceiver();
        if (fee != 0) {
    ...
    function getMetaHatsinCore() public view virtual returns (IMetaHatsinCore) {
        return _getBaseCollVaultStorage()._metaHatsinCore;
    }

    function getPriceFeed() public view virtual returns (IPriceFeed) {
        return IPriceFeed(_getBaseCollVaultStorage()._metaHatsinCore.priceFeed());
    }
