...
interface ILiquidStabilityPool is IERC4626, IERC1822Proxiable {
    struct LSPStorage {
        IMetaHatsinCore metaHatsinCore;
        address feeReceiver;
        ...
        uint64 defaultUnlockRatePerSec;
        EnumerableSet.AddressSet acceptedStables;
        uint uniqueAcceptedStables; // variable to validate preferred tokens arr
        address feeHook;
        address lspSwapper;
        uint minNectReserves; // Minimum NECT reserves required for liquidations
    }
    ...
    struct InitParams {
        ...
        uint64 _defaultUnlockRatePerSecond;
    }
    ...
    function deposit(uint assets, address receiver, address inputToken) external returns (uint shares);
    ...
    function setAcceptedStable(address token, bool add, uint64 unlockRatePerSecond) external;
    function getAcceptedStablesUniqueLength() external view returns (uint256);
    function setFeeHook(address feeHook) external;
    function setLspSwapper(address lspSwapper) external;
    function setMinNectReserves(uint minNectReserves) external;
}
