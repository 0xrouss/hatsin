...
/*  Helper library for grabbing LSP data from offchain. Not part of the core Hatsin system. */
library LSPStorageLib {
    ...
    /* SLOTS */
    uint internal constant META_HATSIN_CORE_SLOT = 0;
    ...
    uint internal constant ACCEPTED_STABLES_SLOT = 15;
    uint internal constant ACCEPTED_STABLES_INDEXES_MAPPING_SLOT = 16;
    ...
    uint internal constant ACCEPTED_STABLES_ARRAY_SLOT = uint(keccak256(abi.encode(LiquidStabilityPoolStorageLocation + ACCEPTED_STABLES_SLOT)));
    ...
    function metaHatsinCoreSlot() internal pure returns (bytes32) {
        return bytes32(LiquidStabilityPoolStorageLocation + META_HATSIN_CORE_SLOT);
    }
    ...
    function acceptedStablesValuesSlot() internal pure returns (bytes32) {
        return bytes32(LiquidStabilityPoolStorageLocation + ACCEPTED_STABLES_SLOT);
    }

    function acceptedStablesArrayValuesSlot(uint index) internal pure returns (bytes32) {
        return bytes32(ACCEPTED_STABLES_ARRAY_SLOT + index);
    }

    function acceptedStablesIndexesMappingSlot(bytes32 value) internal pure returns (bytes32) {
        bytes32 slot = bytes32(LiquidStabilityPoolStorageLocation + ACCEPTED_STABLES_INDEXES_MAPPING_SLOT);
        bytes memory data = abi.encode(value, slot);
        return keccak256(data);
    }
